...
import { buildFields, handlePromiseAll } from '../sync/CassandraSyncRepository';
import { CategoryClient, Channel, channelModel, ChannelSM, Item, ItemSM, ListResult, Playlist,PlaylistCollection,playlistModel,PlaylistSM, PlaylistVideo, StringMap, Video, VideoCategory, videoModel, VideoService, YoutubeSyncClient } from 'video-service';
    ...
    return this.client.execute(query,[regionCode],{prepare:true}).then((category) => {
      if (category.rows[0]) {
        ...
        return this.categoryClient.getCagetories(regionCode).then((r) => {
          ...
          const query =`INSERT INTO ${this.categorisTable} (id,data) VALUES (?,?)`;
          const queries = {
            query,
  ...
  arrayKeys.forEach((key, index) => {
    if (key === "q") {
      ...
      if( arrayValues[index]){
        tableName === "channel" ? must.push({type: "match", field: "id",value: `${arrayValues[index]}`}) : must.push({type: "match", field: `${key.toLowerCase()}`,value: `${arrayValues[index]}`})
      }
    }
    else{
      if (arrayValues[index]){
